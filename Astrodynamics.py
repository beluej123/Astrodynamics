...
mercury = {
    ...
    "e": 0.205630,
    "a": 0.387098 * AU,
    "inc": {"sun_eq": 3.38, "ecliptic": 7.005, "invariable_plane": 6.35},
...
venus = {
    ...
    "e": 0.006772,
    "a": 0.723332 * AU,
    "inc": {"sun_eq": 3.86, "ecliptic": 3.39458, "invariable_plane": 2.15},
...
mars = {
    ...
    "e": 0.0934,
    "a": 2.27939366e11,
    "inc": {"sun_eq": 5.65, "ecliptic": 1.850, "invariable_plane": 1.63},
...
jupiter = {
    ...
    "e": 0.0489,
    "a": 5.2038 * AU,
    "inc": {"sun_eq": 6.09, "ecliptic": 1.303, "invariable_plane": 0.32},
...
saturn = {
    ...
    "e": 0.0565,
    "a": 9.5826 * AU,
    "inc": {"sun_eq": 5.51, "ecliptic": 2.485, "invariable_plane": 0.93},
...
uranus = {
    ...
    "e": 0.04717,
    "a": 19.19126 * AU,
    "inc": {"sun_eq": 6.48, "ecliptic": 0.773, "invariable_plane": 0.99},
...
neptune = {
    ...
    "e": 0.008678,
    "a": 30.07 * AU,
    "inc": {"sun_eq": 6.43, "ecliptic": 1.770, "invariable_plane": 0.74},
...
def eccentricity_vector(
    position: np.ndarray, velocity: np.ndarray, std_grav_param: float
) -> np.ndarray:
    """Calculate eccentricity vector."""
    return np.cross(velocity, np.cross(position, velocity)) / std_grav_param - unit(
        position
    )
...
def semi_latus_rectum(a: float, e: float) -> float:
    ...
    if e == 1:
        raise NotImplementedError(
            "Process of defining parabolic trajectories unavailable."
        )
    if e > 1:
        return -a * (e**2 - 1)
...
def true_2_eccentric_anom(true_anom: float, e: float) -> float:
    ...
    return degrees(
        np.arctan((np.sqrt(1 - e**2) * sp.sindg(true_anom)) / (e + sp.cosdg(true_anom)))
    )
...
def eccentric_2_true_anom(ecc_anom: float, e: float) -> float:
    ...
    return ecc_anom + degrees(
        2 * np.arctan((beta * sp.sindg(ecc_anom)) / (1 - beta * sp.cosdg(ecc_anom)))
    )
...
def mean_2_eccentric_anom(mean_anom: float, e: float) -> float:
    ...
    return mean_anom + degrees(
        2 * sum((sp.jv(n, n * e) / n * sp.sindg(n * mean_anom)) for n in range(1, 250))
    )
...
def mean_2_true_anom(mean_anom: float, e: float) -> float:
    ...
    inner = lambda s: sum(
        (beta**p * (sp.jv(s - p, s * e) + sp.jv(s + p, s * e))) for p in range(1, 150)
    )
    return mean_anom + degrees(
        2
        * sum(
            (sp.sindg(s * mean_anom) / s * (sp.jv(s, s * e) + inner(s)))
...
def mean_2_hyperbolic_anom(mean_hyp_anom: float, e: float) -> float:
    ...
    def f(F):
        return e * np.sinh(F) - F - mean_hyp_anom

    return degrees(
        newton(
            func=f,
            x0=mean_hyp_anom,
            fprime=lambda F: e * np.cosh(F) - 1,
            fprime2=lambda F: e * np.sinh(F),
            maxiter=200,
        )
    )


def true_anom_from_vectors(
    position: np.ndarray, velocity: np.ndarray, std_grav_param: float
) -> float:
    ...
    nu = degrees(
        np.arccos(
            np.dot(ecc_vector, position)
            / np.multiply(*np.linalg.norm((ecc_vector, position), axis=1))
        )
    )
    if np.linalg.norm(ecc_vector) >= 1:
        return -nu if np.dot(position, velocity) < 0 else nu
...
def mean_anomaly_shift(true_anom: float, e: float) -> float:
    ...
    return degrees(
        np.arctan2(-np.sqrt(1 - e**2) * sp.sindg(true_anom), -e - sp.cosdg(true_anom))
        + np.pi
        - e * (np.sqrt(1 - e**2) * sp.sindg(true_anom)) / (1 + e * sp.cosdg(true_anom))
    )
...
def mass_calculator(obj) -> float:
    ...
    if len(obj) > 0:
        total_mass += sum(mass_calculator(satellite) for key, satellite in obj)
    return total_mass
...
def satellite_root(obj) -> str:
    ...
    if hasattr(obj, "primary"):
        root_trace += f"{satellite_root(obj.primary)}"
    if isinstance(obj, Satellite):
...
def namer(obj) -> str:
    ...
    if len(obj) > 0:
        full_name += f"[{', '.join(namer(satellite) for key, satellite in obj)}]system"
    return full_name
...
def plot_energy(obj, y_window=None, r_max=1e8):
    ...
    if obj.e > 1:
        r_peri, r_apo = -obj.a * (obj.e - 1), np.inf
    if obj.e == 1:
        r_peri, r_apo = mag_h**2 / (2 * G * (obj.primary.mass + obj.mass)), np.inf
    if obj.e == 0:
        r_peri, r_apo = obj.a, obj.a
    if obj.e < 1:
        r_peri, r_apo = obj.a * (1 - obj.e), obj.a * (1 + obj.e)
    if np.isinf(r_apo):
        if r_max == None:
            r_max = 10 ** (np.log10(np.ceil(np.abs(r_peri))) + 1)
        r, rs = np.linspace(10, r_max * 1.1), np.linspace(r_peri, r_max * 1.1)
    else:
        r, rs = np.linspace(10, r_apo * 1.1), np.linspace(r_peri, r_apo)

    # CALCULATE POTENTIALS
    U_cf, U_gr = U_centrifugal(r, obj.primary.mass, obj.mass, mag_h), U_gravitational(
        r, obj.primary.mass, obj.mass
    )
    U_cf2, U_gr2 = U_centrifugal(
        rs, obj.primary.mass, obj.mass, mag_h
    ), U_gravitational(rs, obj.primary.mass, obj.mass)
    ...
    (
        ax.axvline(r_apo, dashes=(1, 1), c="cyan", label=r"$r_{apo}$")
        if not r_apo == np.inf
        else None
    )
    ax.axvline(r_0, dashes=(1, 1), c="magenta", label=r"$r_{circ}$")
    ax.set_title(
        f"Effective Potential of {obj.body.name} About {obj.primary.body_name if isinstance(obj.primary, System) else obj.primary.body.name}"
    )
    ...
    ax.fill_between(rs, np.full_like(rs, obj.specific_energy), U_eff2, alpha=0.25)
    if np.isinf(r_apo):
        ax.set_xticks(
            [0, r_peri, r_0, r.max()], [0, r"$r_{peri}$", r"$r_{circ}$", "max"]
        )
    else:
        ax.set_xticks(
            [0, r_peri, r_0, r_apo, r.max()],
            [0, r"$r_{peri}$", r"$r_{circ}$", r"$r_{apo}$", "max"],
        )
...
def makefig(graphic_type: str, scale: str = "AU"):
    ...
    if not scale in spatial_conversion.keys():
        raise KeyError(f"Spatial scale {scale} not defined.")
    ...
    if graphic_type == "planar":
        fig, axes = plt.subplots(
            ...
            sharex="col",
            figsize=(10, 10),
            gridspec_kw={"hspace": 0.035, "wspace": 0.025},
        )
    ...
    fig.suptitle("Predicted Trajectories of Provided System", y=0.95)
...
def central_obj(fig, axes, graphic_type: str, primary_obj, scale: str = "AU"):
    ...
    if graphic_type == "planar":
        ...
        for i, ax in enumerate([ax1, ax2, ax3, ax4]):
            ax.add_patch(
                plt.Circle((0, 0), radius, color=primary_obj.color, alpha=0.75)
            )
            if isinstance(primary_obj, Satellite):
                ax.add_patch(
                    plt.Circle(
                        (0, 0),
                        primary_obj.SOI / scale_factor,
                        color="red",
                        fill=False,
                        linestyle=":",
                    )
        ...
        if isinstance(primary_obj, Satellite):
            axes.text(
                0, 0, 0, "%s" % primary_obj.body.name, size=8, zorder=1, color="white"
            )
        if isinstance(primary_obj, System):
            axes.text(0, 0, 0, "%s" % primary_obj.name, size=8, zorder=1, color="white")
        axes.plot_surface(*sphere(radius), color=primary_obj.color, alpha=1)
        if isinstance(primary_obj, Satellite):
            axes.plot_wireframe(
                *sphere(primary_obj.SOI / scale_factor), alpha=0.25, color="red"
            )
    return [fig, axes, graphic_type]
...
def plot_satellite(
    ...
    if isinstance(relative_to, int):
        ...
        if relative_to.body.name == sat_obj.primary.body.name:
            sat_coords = (
                sat_obj.prediction.local_coords[::markevery, :].T / scale_factor
            )

    r0, rf = (
        sat_obj.prediction.local_coords[0, :] / AU,
        sat_obj.prediction.local_coords[-1, :] / AU,
    )
    x0, y0, z0, xf, yf, zf = *sat_coords[:, 0], *sat_coords[:, -1]
    if isinstance(sat_obj.primary, Satellite):
        soi_0, soi_f = np.apply_along_axis(
            r_soi,
            0,
            np.linalg.norm((r0, rf), axis=1),
            *[sat_obj.primary.body.mass, sat_obj.body.mass],
        )
        ...
        soi_0, soi_f = np.apply_along_axis(
            ...
            0,
            np.linalg.norm((r0, rf), axis=1),
            *[sat_obj.primary.body_mass, sat_obj.body.mass],
        )
    if graphic_type == "planar":
        ((ax1, ax2), (ax3, ax4)) = axes
        ...
        pos_0, pos_f = [(x0, y0), (z0, y0), (x0, z0), (z0, y0)], [
            (xf, yf),
            (zf, yf),
            (xf, zf),
            (zf, yf),
        ...
        for i, ax in enumerate([ax1, ax2, ax3, ax4]):
            ax.add_patch(
                plt.Circle(pos_0[i], soi_0, fill=False, linestyle=":", color="magenta")
            )
            ax.add_patch(
                plt.Circle(pos_f[i], soi_f, fill=False, linestyle=":", color="red")
            )
    elif graphic_type == "3d":
        cb = axes.scatter3D(*sat_coords, c=time, cmap="viridis", s=0.5)
        xs, ys, zs = sphere(soi_0)
        axes.plot_wireframe(xs + x0, ys + y0, zs + z0, alpha=0.25, color="magenta")
        xs, ys, zs = sphere(soi_f)
        axes.plot_wireframe(xs + xf, ys + yf, zs + zf, alpha=0.25, color="red")
    ...
    if cbar:
        fig.colorbar(
            ...
            label=f"Estimated satellite position after X {timestep}",
            fraction=0.0335,
        )
    return [fig, axes, graphic_type]
...
def window_polish(
    fig, axes, graphic_type, x_window=None, y_window=None, z_window=None
) -> list:
    """
    ...
    if graphic_type == "planar":
        ...
        if isinstance(z_window, list):
            for ax in [ax2, ax4]:
                ax.set_xlim(z_window[0], z_window[1])
        ...
        (
            None
            if x_window is None
            else axes.axes.set_xlim3d(left=x_window[0], right=x_window[1])
        )
        (
            None
            if y_window is None
            else axes.axes.set_ylim3d(bottom=y_window[0], top=y_window[1])
        )
        (
            None
            if z_window is None
            else axes.axes.set_zlim3d(bottom=z_window[0], top=z_window[1])
        )
...
def keplerian_2_cartesian(
    std_grav_param: float, kwargs: dict[str, float], inclination: str = "ecliptic"
) -> list:
    """
    ...
    if not isinstance(kwargs, dict):
        raise TypeError("Keplerian elements must be defined in a dictionary.")
    a, e = check_sign(kwargs["a"], kwargs["e"])
    if (a == 0) or (e == 1):
        raise NotImplementedError(
            "Process for defining parabolic trajectories from Keplerian elements not yet available, try providing state vectors instead."
        )
    try:
        angles = (
            kwargs["arg_periapsis"],
            kwargs["inc"][inclination],
            kwargs["long_asc_node"],
        )
    except:
        ...
        r_w = r * np.array(
            [sp.cosdg(kwargs["true_anomaly"]), sp.sindg(kwargs["true_anomaly"]), 0]
        )
        v_w = (
            std_grav_param
            / h
            * np.array(
                [
                    -sp.sindg(kwargs["true_anomaly"]),
                    e + sp.cosdg(kwargs["true_anomaly"]),
                    0,
                ]
            )
        )
        return Rotation.from_euler(
            "ZXZ", [-angle for angle in angles], degrees=True
        ).apply([r_w, v_w], inverse=True)
...
def cartesian_2_keplerian(std_grav_param: float, vectors: dict[str, float]) -> list:
    ...
    if not isinstance(vectors, dict):
        raise TypeError("State vectors must be defined in a dictionary")
    if (vectors["position"].shape != (3,)) or (vectors["velocity"].shape != (3,)):
        raise ValueError(f"State vector shapes must be (3, )")
    position, velocity = vectors["position"], vectors["velocity"]
    h_vector = np.cross(position, velocity)
    specific_angular_momentum, r, v = np.linalg.norm(
        (h_vector, position, velocity), axis=1
    )
    ...
    if N[1] >= 0:
        long_asc = np.arccos(N[0] / np.linalg.norm(N))
    if N[1] < 0:
        long_asc = 2 * np.pi - np.arccos(N[0] / np.linalg.norm(N))
    if e_vector[2] >= 0:
        arg_periapsis = np.arccos(np.dot(e_vector, N) / (e * np.linalg.norm(N)))
    if e_vector[2] < 0:
        arg_periapsis = 2 * np.pi - np.arccos(
            np.dot(e_vector, N) / (e * np.linalg.norm(N))
        )
    return (
        a,
        e,
        semi_circ(degrees(inc)),
        degrees(arg_periapsis),
        degrees(long_asc),
        true_anom_from_vectors(position, velocity, std_grav_param),
    )
...
class Body:
    ...
    def __init__(self, data: dict[str, float], negligible: bool = False):
        ...
        if not "name" in data.keys():
            raise KeyError("`name` must be specified.")
        self.body_name = data["name"]
        if not "mass" in data.keys():
            raise KeyError("`mass` must be specified.")
    ...
    def __round__(self, n):
        return Body(self.name, round(self.mass, n), round(self.radius, n))
    ...
    def __ne__(self, other):
        if self.negligible or other.negligible:
            return False if self.negligible and other.negligible else True
        return self.mass != other.mass

    def __eq__(self, other):
        if self.negligible or other.negligible:
            return True if self.negligible and other.negligible else False
        return self.mass == other.mass

    def __gt__(self, other):
        if self.negligible or other.negligible:
            if self.negligible and other.negligible:
                return False
...
class System:
    """
    System class represents a system of bodies.
    ...
    def __iter__(self):
        return iter(self.satellites.items())
    ...
    def __contains__(self, item):
        if isinstance(item, str):
            for sat_name, sat_obj in self:
                if (sat_name == item) or (sat_obj.name == item):
                    return True
                if len(sat_obj) > 0:
                    return item in sat_obj
        elif isinstance(item, Body) or isinstance(item, Satellite):
            for sat_name, sat_obj in self:
                if (sat_obj.body_name == item.body_name) or (
                    sat_obj.name == item.body_name
                ):
                    return True
                if len(sat_obj) > 0:
                    item in sat_obj
    ...
    def __get_pred_lengths(self) -> np.ndarray:
        ...
        for satname, sat_obj in self:
            ...
            if len(sat_obj) > 0:
                pred_lengths = np.concatenate(
                    (pred_lengths, sat_obj.get_pred_lengths())
                )
    ...
    def __truncate(self, lengths: np.ndarray):
        ...
        for satname, sat_obj in self:
            sat_obj.force_truncate(lengths.min())
    ...
    def __check_encounters(self):
        ...
        while len(_satellites) > 1:
            ...
            for satX in _satellites:
                ...
                if not satA.negligible:
                    distance = np.subtract(distance, satA.SOI)
                if not satX.negligible:
                    distance = np.subtract(distance, satX.SOI)
                if np.any(np.nonzero(distance < 0.0)):
                    idx = np.where(distance < 0.0)[0][0]
                    print(
                        f"Encounter found between {satA.body.name} and {satX.body.name} at time {satA.prediction.time_array[idx] / timestep_conversion['days']} days"
                    )
                    encounter_indices.append(idx)
        if len(encounter_indices) > 0:
            self.__truncate(np.array(encounter_indices))
    ...
    def add_satellite(self, secondary: Body, kwargs: dict[str, float]):
        ...
        if self.primary > secondary:
            self.satellites[secondary.name] = Satellite(
                secondary, kwargs, primary=self, inclination=self.inclination
            )
            self.system_name = self.name
            self.system_mass = self.mass
        else:
    ...
    def simulate(
        ...
        solver: str = "best",
        check_escape: bool = True,
        check_collision: bool = False,
        check_encounter: bool = True,
    ):
        """
        Predicts the motion of the satellite about the primary celestial body, and returns results of the simulation as an attribute of the simulated satellite.
        ...
        if (not np.isfinite(duration)) or (0 > duration):
            raise ValueError(
                "Simulation duration must be a finite, nonnegative number."
            )
        if not timestep in timestep_conversion.keys():
            raise KeyError(f"Timestep conversion not defined for timestep {timestep}.")
        times = np.linspace(0, duration * timestep_conversion[timestep], resolution)

        for satname, sat_obj in self:
            sat_obj.prediction = Predict(
                sat_obj,
                times,
                solver=solver,
    ...
    def plot_simulation(
        ...
        plt.close()
        has_cbar = False
        if not graphic_type in ["3d", "planar"]:
            raise ValueError("Graphic type not '3d' or 'planar'.")
        if not scale in spatial_conversion.keys():
            raise KeyError(f"Spatial conversion scale {scale} not found.")
        if not timestep in timestep_conversion.keys():
            raise KeyError(f"Timestep conversion {timestep} not found.")

        figobj = makefig(graphic_type, scale=scale)
        figobj = central_obj(*figobj, self.primary, scale=scale)
        for satname, sat_obj in self:
            if not hasattr(sat_obj, "prediction"):
                raise AttributeError(f"Satellite {satname} has not been simulated.")
            figobj = plot_satellite(
                *figobj,
                sat_obj,
                scale=scale,
                markevery=markevery,
        ...
        figobj = window_polish(
            *figobj, x_window=x_window, y_window=y_window, z_window=z_window
        )
        plt.show()
...
class Trajectory(Body):
    """
    Trajectory class represents a static trajectory.
    ...
    def __init__(
        self, kwargs: dict[str, float], to_set: dict = {}, inclination: str = "ecliptic"
    ):
        """
        Parameters
        ----------
        ...
        if not isinstance(kwargs, dict):
            raise TypeError(
                "Trajectory object can only be created using a dictionary of parameters"
            )
        self.__inclination = inclination
        default = {
            "a": 1.0,
        ...
        if len(to_set) == 0:
            if any(element in kepler for element in kwargs) and any(
                element in cartes for element in kwargs
            ):
                raise NotImplementedError(
                    "Process to set a trajectory using both Keplerian and Cartesian parameters does not exist yet."
                )
    ...
    def __repr__(self):
        return f"{self.orbit} Trajectory - a:{self.a}m, e:{self.e}, inc:{self.inc}deg"

    def __str__(self):
        return f"{self.orbit} Trajectory - a:{self.a}m, e:{self.e}, inc:{self.inc}deg"

    def keys(self):
        return ["orbit", *kepler, *cartes]
    ...
    def _from_kepler_(self, kwargs: dict[str, float]):
        if kwargs["e"] < 0:
            raise ValueError("Eccentricity cannot be less than zero.")
        a, e = check_sign(kwargs["a"], kwargs["e"])
        position, velocity = keplerian_2_cartesian(
            self.gparam, kwargs, inclination=self.__inclination
        )
    ...
    def _from_state_vectors_(self, kwargs: dict[str, np.ndarray]):
        a, e, inc, arg_periapsis, long_asc_node, true_anomaly = cartesian_2_keplerian(
            self.gparam, kwargs
        )
        (
            self.orbit,
    ...
    def __placeholder(self, original, to_set):
        ...
        if any(element in kepler for element in to_set) and any(
            element in cartes for element in to_set
        ):
            raise NotImplementedError(
                "Process to set a trajectory using both Keplerian and Cartesian parameters does not exist yet."
            )
            ...
            self._from_state_vectors_(
                {element: original[element] for element in cartes}
            )
    ...
    def __align_and_rotate(self, position, a, e):
        ...
        r_w = r * np.array(
            [sp.cosdg(self.true_anomaly), sp.sindg(self.true_anomaly), 0]
        )
        v_w = (
            self.gparam
            / np.linalg.norm(self.specific_angular_momentum)
            * np.array(
                [-sp.sindg(self.true_anomaly), e + sp.cosdg(self.true_anomaly), 0]
            )
        )
    ...
    def radial_velocity(self) -> float:
        return radial_velocity(self.position, self.velocity)
    ...
    def gparam(self) -> float:
        ...
        if isinstance(self.primary, System):
            return grav_param(self.primary.primary.mass, self.body.mass)
    ...
    def excess_velocity(self) -> float:
        ...
        if self.orbit == "Hyperbolic":
            return np.sqrt(-self.gparam / self.a)
    ...
    def mean_angular_motion(self) -> float:
        ...
        if self.e == 1:
            return mean_angular_motion(self.gparam)
    ...
    def flight_path_angle(self) -> float:
        ...
        if self.orbit == "Parabolic":
            return self.true_anomaly / 2
        true_anomaly = radians(self.true_anomaly)
        return degrees(
            np.arctan(
                (self.e * sp.sindg(true_anomaly))
                / (1 + self.e * sp.cosdg(true_anomaly))
            )
        )
    ...
    def rotation(self):
        ...
        return Rotation.from_euler(
            ...
            [
                -getattr(self, angle)
                for angle in ["arg_periapsis", "inc", "long_asc_node"]
            ],
            degrees=True,
        )
    ...
    def specific_energy(self) -> float:
        ...
        if self.e < 0:
            raise ValueError("Eccentricity less than zero")
        if self.e == 1:
            return 0
...
class Satellite(Trajectory):
    ...
    def __repr__(self):
        return f"Satellite of {satellite_root(self.primary)} - {self.body.name}"

    def __str__(self):
        return f"Satellite of {self.primary.name} - {self.body.name}"

    def keys(self):
        return ["body", "orbit", "primary", *kepler, *cartes]
    ...
    def __contains__(self, item):
        if isinstance(item, str):
            for sat_name, sat_obj in self:
                if (sat_name == item) or (sat_obj.body.name == item):
                    return True
                if len(sat_obj) > 0:
                    return item in sat_obj
        elif isinstance(item, Body) or isinstance(item, Satellite):
            for sat_name, sat_obj in self:
                if (sat_obj.body.name == item.body.name) or (sat_obj.name == item.name):
                    return True
                if len(sat_obj) > 0:
                    return item in sat_obj
        return False
        ...
        while len(_satellites) > 1:
            ...
            for satX in _satellites:
                ...
                if np.any(np.nonzero(distance < 0.0)):
                    idx = np.where(distance < 0.0)[0][0]
                    print(
                        f"Encounter found between {satA.body_name} and {satX.body_name} at index {idx}"
                    )
                    encounter_indices.append(idx)
        if len(encounter_indices) > 0:
            self.force_truncate(np.array(encounter_indices).min())
    ...
    def set_elements(self, kwargs: dict[str, float]):
        ...
        if any(element in kepler for element in kwargs) and any(
            ...
            if all(element in kwargs for element in cartes):
                ...
                super().__init__(
                    {"position": kwargs["position"], "velocity": kwargs["velocity"]},
                    inclination=self.__inclination,
                )
                ...
                raise NotImplementedError(
                    "Process to set a trajectory using both Keplerian and Cartesian parameters currently unnavailable."
                )
        elif any(element in kepler for element in kwargs):
            super().__init__(
                self.elements, to_set=kwargs, inclination=self.__inclination
            )
        elif any(element in cartes for element in kwargs):
            super().__init__(
                self.elements, to_set=kwargs, inclination=self.__inclination
    ...
    def set_semimajor_axis(self, a: float):
        ...
        elements = self.kepler
        elements["a"] = a
        if (self.e > 1) and (a > 0):
            a *= -1
        if (self.e < 1) and (a < 0):
            raise ValueError(
                "Semi-major axis < 0 implies hyperbolic trajectory but eccentricity does not match."
            )
    ...
    def set_inclination(self, inc: float):
        ...
        super().__init__(
            self.elements, to_set={"inc": inc}, inclination=self.__inclination
        )
    ...
    def set_arg_periapsis(self, arg_periapsis: float):
        ...
        super().__init__(
            self.elements,
            to_set={"arg_periapsis": arg_periapsis},
            inclination=self.__inclination,
        )
    ...
    def set_long_asc_node(self, long_asc_node: float):
        ...
        super().__init__(
            self.elements,
            to_set={"long_asc_node": long_asc_node},
            inclination=self.__inclination,
        )
    ...
    def set_true_anomaly(self, anomaly: float):
        ...
        if self.orbit == "Hyperbolic":
            ...
            if np.abs(anomaly) > true_anom_inf:
                raise ValueError(
                    f"Anomaly for this trajectory cannot not exceed {true_anom_inf} degrees"
                )
        if self.orbit == "Parabolic":
            if np.abs(anomaly) > 180:
                raise ValueError("Anomaly must not exceed 180 degrees.")
        super().__init__(
            self.elements,
            to_set={"true_anomaly": anomaly},
            inclination=self.__inclination,
        )
    ...
    def set_eccentric_anomaly(self, anomaly: float):
        ...
        elements = self.kepler
        elements["true_anomaly"] = eccentric_2_true_anom(anomaly, self.e)
        super().__init__(
            self.elements,
            to_set={"true_anomaly": eccentric_2_true_anom(anomaly, self.e)},
            inclination=self.__inclination,
        )
    ...
    def set_position(self, position: np.ndarray):
        ...
        if position.shape != (3,):
            raise ValueError(f"Shape of position must be (3, )")
        super().__init__(
            {"position": position, "velocity": self.velocity},
            inclination=self.__inclination,
        )
    ...
    def set_velocity(self, velocity: np.ndarray):
        ...
        if velocity.shape != (3,):
            raise ValueError(f"Shape of velocity must be (3, )")
        super().__init__(
            {"position": self.position, "velocity": velocity},
            inclination=self.__inclination,
        )
    ...
    def add_satellite(self, secondary_body: Body, kwargs: dict[str, float]):
        ...
        if not self.primary > secondary_body:
            raise ValueError("Secondary more massive than primary.")
        self.satellites[secondary_body.name] = Satellite(
            secondary_body, kwargs, primary=self, inclination=self.__inclination
        )
    ...
    def simulate(
        ...
        Predicts the motion of the satellite about the primary celestial body,
            and returns results of the simulation as an attribute of the simulated satellite.
        ...
        duration : float or int
            Length of time to simulate a trajectory in `timestep`.
        timestep : str
            Conversion factor from duration in `timestep` to duration in seconds.
            Defined timesteps: `seconds`, `minutes`, `hours`, `days`, `years` (default: `days`).
        resolution : int
            Number of equally-spaced times between zero and `duration`.
        solver : str
            Method used to predict the motion of the satellite (default: 'best').
        check_escape : bool
            If `True`, all radial distances will be compared to the system's sphere of influence.
            If the radial distance exceeds the SOI, all data up to the time where the satellite
                leaves the system's SOI will be returned (default: `True`).
        check_escape : bool
            If `True`, all radial distances will be compared to the radius of the primary body.
            If the radial distance does not exceed the primary body's radius, all data up to
                the time where the satellite collides with the primary will be returned (default: `False`).
        check_encounter : bool
    ...
    def get_pred_lengths(self) -> np.ndarray:
        ...
        for satname, sat_obj in self:
            ...
            if len(sat_obj) > 0:
                predict_lengths = np.concatenate(
                    (predict_lengths, sat_obj.get_pred_lengths())
                )
...
class Predict:
    """
    Predict class represents satellite motion in the system with respect
        to the immediate primary.
        You may also google search "python how to build an orbit propagator"
            for other ideas.
    ...
    def __init__(
        ...
        solver : str
            Method used to predict the motion of the satellite (default: 'best').
        check_escape : bool
            If `True`, all radial distances will be compared to the system's sphere
                of influence. If the radial distance exceeds the SOI, all data up to
                the time where the satellite leaves the system's SOI will be returned (default: `True`).
        check_escape : bool
            If `True`, all radial distances will be compared to the radius of the
                primary body. If the radial distance does not exceed the primary body's
                radius, all data up to the time where the satellite collides with the
                primary will be returned (default: `False`).
        ...
        if satellite.primary < satellite:
            raise ValueError("Primary body more massive than satellite.")
        ...
        if solver == "best":
            if satellite.orbit == "Circular":
                ...
                self.__iternomaly(
                    time_array, degrees(satellite.mean_angular_motion * time_array)
                )
                ...
                self.__iternomaly(
                    time_array,
                    mean_2_true_anom(
                        mean_anomaly_shift(satellite.true_anomaly, satellite.e)
                        + degrees(satellite.mean_angular_motion * time_array),
                        satellite.e,
                    ),
                )
                ...
                F_0 = np.sign(self.__satellite.true_anomaly) * np.arccosh(
                    (self.__satellite.e + sp.cosdg(self.__satellite.true_anomaly))
                    / (1 + self.__satellite.e * sp.cosdg(self.__satellite.true_anomaly))
                )
                ...
                """
                Need to store the indices where the anomaly is negative to
                    accurately convert the hyperbolic to true anomaly. This
                    also serves to establish an intuitive interpretation of
                    where the satellite is along its trajectory with respect
                    to pericenter.
                """
                if self.__satellite.radial_velocity < 0:
                    isneg = np.greater(
                        hyp_anoms,
                        np.full_like(
                            hyp_anoms, degrees(np.arccos(-1 / self.__satellite.e))
                        ),
                    )
                    hyp_anoms[isneg] -= 360
                    true_anoms = np.apply_along_axis(
                        hyperbolic_2_true_anom, 0, np.abs(hyp_anoms), self.__satellite.e
                    )
                    true_anoms[isneg] *= -1
                else:
                    true_anoms = np.apply_along_axis(
                        hyperbolic_2_true_anom, 0, hyp_anoms, self.__satellite.e
                    )
            ...
            raise KeyError(
                f"Prediction method {solver} not defined. Use 'best' or 'integrate'."
            )
        ...
        if check_escape:
            if isinstance(self.__primary, System):
                ...
                radial_vel = np.array(
                    [
                        radial_velocity(pos, vel)
                        for pos, vel in zip(self.local_coords, self.local_velocity)
                    ]
                )
                wrt_soi = np.greater(
                    self.radial, np.full_like(self.time_array, self.__primary.SOI)
                )
                ...
                if np.any(np.nonzero(np.logical_and(wrt_soi, diff_radial))):
                    ...
                    print(
                        f"Satellite {self.object} escapes primary {self.__primary.body.name} at time {time_array[idx] / timestep_conversion['days']} days"
                    )
                    self.truncate(idx)
        if check_collision:
            if isinstance(self.__primary, System):
                if np.any(
                    np.nonzero(
                        self.radial
                        < np.full_like(self.time_array, self.__primary.primary.radius)
                    )
                ):
                    idx = np.where(
                        self.radial
                    ...
                    print(
                        f"Satellite {self.object} impacts primary {self.__primary.body.name} at time {time_array[idx] / timestep_conversion['days']} days"
                    )
                    self.truncate(idx)
            else:
                if np.any(
                    np.nonzero(
                        self.radial
                        < np.full_like(self.time_array, self.__primary.radius)
                    )
                ):
                    idx = np.where(
                        self.radial
    ...
    def radial(self) -> np.ndarray:
        return np.linalg.norm(self.local_coords, axis=1)

    @property
    def speed(self) -> np.ndarray:
        return np.linalg.norm(self.local_velocity, axis=1)
    ...
    def global_coords(self) -> np.ndarray:
        ...
        if not hasattr(self.__primary, "prediction"):
            return self.local_coords + np.full(
                (
                    len(self),
                    3,
                ),
                self.__primary.position,
            )
    ...
    def global_velocity(self) -> np.ndarray:
        ...
        if not hasattr(self.__primary, "prediction"):
            return self.local_velocity + np.full(
                (
                    len(self),
                    3,
                ),
                self.__primary.velocity,
            )
    ...
    def __iternomaly(self, time_array: np.ndarray, true_anoms: np.ndarray):
        ...
        self.local_coords, self.local_velocity = np.full(
            (
                len(self),
                3,
            ),
        ...
        keplerian_elements = {
            element: getattr(self.__satellite, element) for element in kepler
        }
        for idx, true_anom in enumerate(self.true_anomaly):
            keplerian_elements["true_anomaly"] = (
                true_anom if keplerian_elements["e"] < 1 else -true_anom
            )
            self.local_coords[idx, :], self.local_velocity[idx, :] = (
                keplerian_2_cartesian(self.__satellite.gparam, keplerian_elements)
            )
        if self.__satellite.e > 1:
            self.local_velocity *= -1
    ...
    def __integrate(self, time_array: np.ndarray):
        ...
        state = (
            np.array([*self.__satellite.position.T, *self.__satellite.velocity.T])
            * 1e-3
        )
        solution = (
            odeint(
                ...
                state,
                self.time_array,
                args=(self.__satellite.gparam * 1e-9,),
            )
            * 1e3
        )
        local_coords, local_velocity = solution[:, :3], solution[:, 3:]
        self.true_anomaly = np.array(
            [
                true_anom_from_vectors(pos, vel, self.__satellite.gparam)
                for pos, vel in zip(local_coords, local_velocity)
            ]
        )
...
def get_system(func):
    ...
    def get_primary(obj):
        while not isinstance(obj, System):
            return get_primary(getattr(obj, "primary"))
        return deepcopy(obj)
...
def update_system(func):
    ...
    def update_satellite_system(satsys):
        for satname, sat_obj in satsys:
            elements = sat_obj.kepler
            elements["true_anomaly"] = sat_obj.prediction.true_anomaly[-1] % 360
            state_vectors = {
                "position": sat_obj.prediction.local_coords[-1, :],
                "velocity": sat_obj.prediction.local_velocity[-1, :],
            }
            del sat_obj.prediction
            sat_obj.set_elements(elements if elements["e"] < 1 else state_vectors)
            if len(sat_obj) > 0:
                update_satellite_system(sat_obj)

    def wrapper(system, satA, satB):
        update_satellite_system(system)
        try:
            system = func(
                system.satellites[satA.body.name],
                system.satellites[satB.body.name],
                system=system,
            )
        except KeyError:
            system = func(
                system.satellites[satA.body.name],
                system.satellites[satA.body.name].satellites[satB.body.name],
                system=system,
            )
        finally:
...
def encounter(satA, satB, system=None):
    ...
    system.satellites[satA.body.name].add_satellite(
        satB.body, {"position": rel_posB, "velocity": rel_velB}
    )
...
def solar_system(inclination="sun_eq"):
    ...
    for planet in [*terrans, *giants]:
        solsys.add_satellite(Body(planet), planet)
